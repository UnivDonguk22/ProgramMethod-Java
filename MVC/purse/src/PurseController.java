public class PurseController {
	...
	public void processTransactions() {
		...
		message = "+,-, > , < 키 중 하나를 누르고 OK 단추를 누르세요. (+ : 수입, - : 지출, > : 한일, < : 일한)";
		...
		// If input 'K || k'
			...
			if (request == '<') {
				...
				if (purseJapan.spend(amount)) {
					amount = amount * 10;
					purseKorea.earn(amount);
					outputJapan.showTransaction("환전 지출");
					outputKorea.showTransaction("환전 수입");
				}
				else {
					outputJapan.showTransaction("환전 실패");
				}
			}
			else if (request == '>') {
				message = "금액을 입력하고 OK 단추를 누르세요.";
